...
#####
# added new mapping metrics 5/29/2026
#####
...
for(k in 1:2){
  ...
  print(paste0("Plotting maps for: ", stateName[k]))
    ...
    heavyRainDays050 <- calc(gridStack, fun=function(x){
      sum(!is.na(x) & x >= 0.50, na.rm = TRUE)
    })
    heavyRainDays050[heavyRainDays050 <= 0] <- NA

    # longest wet spell using 0.05" event threshold
    longestWetSpell <- calc(gridStack, fun=function(x){
      wet <- !is.na(x) & x >= 0.05
      r <- rle(wet)
      if(any(r$values)) {
        max(r$lengths[r$values])
    ...
    longestWetSpell[longestWetSpell <= 0] <- NA

    # precipitation concentration / burstiness:
    # percent of seasonal precipitation from the wettest 5 days
    burstinessTop5 <- calc(gridStack, fun=function(x){
      x <- x[!is.na(x)]
      total <- sum(x, na.rm = TRUE)
      if(length(x) == 0 || total <= 0) {
        return(NA)
      }
      top5 <- sum(sort(x, decreasing = TRUE)[1:min(5, length(x))], na.rm = TRUE)
      (top5 / total) * 100
    })
    burstinessTop5[burstinessTop5 <= 0] <- NA
    ...
    rm(plot, logo_raw, logo, final_plot)
    gc()
    ...
    # helper function for additional monsoon summary maps ----
    plot_monsoon_raster <- function(r, file_suffix, title_text, legend_name,
                                    limits, breaks, labels, colors,
                                    output_res = 300L) {

      prec_df <- as.data.frame(r, xy = TRUE, na.rm = FALSE)
      colnames(prec_df) <- c("x", "y", "value")

      p <- ggplot() +
        geom_raster(data = prec_df, aes(x = x, y = y, fill = value)) +
        scale_fill_gradientn(colours = colors, na.value = "burlywood4",
                             name = legend_name, limits = limits, oob = squish,
                             breaks = breaks, labels = labels, expand = NULL) +
      ...
      p <- p + geom_point(data = SWCities, aes(x = lon, y = lat),
                          size = 1, shape = 20)

      p <- p + geom_text(data = SWCities, aes(x = lon, y = lat, label = City),
                         size = 3, col = "black", fontface = "bold", nudge_y = 0.1)

      p <- p + geom_sf(data = goc, fill = "powderblue",
                       color = "black", size = .25) +
        coord_sf(xlim = xlim, ylim = ylim, expand = FALSE)

      out_file <- paste0("/home/crimmins/RProjects/StateMonsoonMaps/maps/",
                         stateAbb[k], "/", stateAbb[k], file_suffix)

      png(out_file, width = 8, height = 8, units = "in", res = output_res)
      print(p, newpage = FALSE)
      dev.off()

      plot <- image_read(out_file)
      logo_raw <- image_read("/home/crimmins/RProjects/logos/UA_CSAP_CLIMAS_logos_horiz.png")
      logo <- image_resize(logo_raw, geometry_size_percent(width = 120, height = 120))
      final_plot <- image_composite(plot, logo, offset = "+110+2150")
      image_write(final_plot, out_file)
      rm(plot, logo_raw, logo, final_plot)
      gc()
    }

    # NEW MAP: Longest dry spell ----
    precipCols <- colorRampPalette(c("deepskyblue3", "cyan", "palegreen1",
                                     "khaki", "yellow", "orange",
                                     "orangered", "red", "darkred"))(50)
    precBreaks <- seq(0, 60, 5)
    precLabs <- as.character(seq(0, 60, 5))
    precLabs[length(precLabs)] <- ">60"

    plot_monsoon_raster(
      r = longestDrySpell,
      file_suffix = "_Monsoon_LongestDrySpell.png",
      title_text = paste0("Longest Dry Spell (consec. days <0.05 in/day): ",
                          allDates[1], " to ", allDates[length(allDates)]),
      legend_name = "days",
      limits = c(0, 60),
      breaks = precBreaks,
      labels = precLabs,
    ...
    # NEW MAP: Heavy rain days >=0.50" ----
    precipCols <- colorRampPalette(c("lightblue", "dodgerblue3", "palegreen",
                                     "green4", "yellow", "orange",
                                     "orangered3", "red4", "purple"))(50)
    precBreaks <- seq(0, 20, 2)
    precLabs <- as.character(seq(0, 20, 2))
    precLabs[length(precLabs)] <- ">20"

    plot_monsoon_raster(
      r = heavyRainDays050,
      file_suffix = "_Monsoon_HeavyRainDays050.png",
      title_text = paste0("Heavy Rain Days (>=0.50 in/day): ",
                          allDates[1], " to ", allDates[length(allDates)]),
      legend_name = "days",
      limits = c(0, 20),
      breaks = precBreaks,
    ...
    plot_monsoon_raster(
      r = longestWetSpell,
      file_suffix = "_Monsoon_LongestWetSpell.png",
      title_text = paste0("Longest Wet Spell (consec. days >=0.05 in/day): ",
                          allDates[1], " to ", allDates[length(allDates)]),
      legend_name = "days",
      limits = c(0, 20),
      breaks = precBreaks,
    ...
    # NEW MAP: Top-5-day burstiness ----
    precipCols <- colorRampPalette(c("darkgreen", "green3", "yellow",
    ...
    plot_monsoon_raster(
      r = burstinessTop5,
      file_suffix = "_Monsoon_Top5Burstiness.png",
      title_text = paste0("Precip Concentration (% from Wettest 5 Days): ",
                          allDates[1], " to ", allDates[length(allDates)]),
      legend_name = "%",
      limits = c(0, 100),
      breaks = precBreaks,
  ...
  #  p<-gplot(totalPrecipAll) + geom_tile(aes(fill = value)) +
    ...
    p<-p+geom_sf(data = goc, fill = "powderblue", color = 'black', size = .25)+
      ...
      render(paste0('/home/crimmins/RProjects/StateMonsoonMaps/maps/',stateAbb[k],"/",stateAbb[k],'MonsoonTemplate_v2.Rmd'), output_file=paste0(tolower(stateAbb[k]),'_monsoon.html'),
             output_dir=paste0('/home/crimmins/RProjects/StateMonsoonMaps/maps/',stateAbb[k],"/"), clean=TRUE)
      # ----  
