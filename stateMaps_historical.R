...
for(l in 2017:2021){
    ...
    dateRangeStart=paste0(l,"-06-15")
    dateRangeEnd=paste0(l,"-09-30") ### SET FOR TESTING
    ...
    # get current year
    currYR<-format(allDates[1],"%Y")
    # make directory
    dir.create(paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR),showWarnings = FALSE)
    ...
    for(k in 1:2){
      ...
      # make state dir in currYr
      dir.create(paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k]),showWarnings = FALSE)
        ...
        # write out file
        png(paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_Anomaly_TS.png"), width = 10, height = 8, units = "in", res = 300L)
        #grid.newpage()
        ...
        image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_Anomaly_TS.png"))
        # ----
        ...
        # And overwrite the plot without a logo
        image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_TotalPrecip.png"))
        #####
        
        # #####
        # # LONG-TERM average precipitation map (normals, needs updating every 10 years)
        # 
        # # LONG-TERM average precipitation map (normals, needs updating every 10 years) -----
        # # colorramp for total precip
        # precipCols<-colorRampPalette(c("lightblue", "dodgerblue3", "palegreen","green4","salmon","orangered3",
        ...
        # #theme_set(theme_bw())
        # 
        # p<-gplot(normal) + geom_tile(aes(fill = value)) +
        #   #scale_fill_gradient2(low = 'white', high = 'blue') +
        #   #scale_fill_distiller(palette = "Spectral", direction = -1, na.value="darkgoldenrod", 
        #   #                     name="inches", limits=c(0,20),oob=squish)+
        #   
        ...
        #   #scale_x_continuous(breaks = c(-120,-140))+
        #   #ggtitle("Daily Total Precip (inches) - PRISM")+
        #   ggtitle(paste0("Average Total Precipitation (in): June 15th-Sept 30th (1991-2020)"))+
        #   labs(caption=paste0("Plot created: ",format(Sys.time(), "%Y-%m-%d"),
        #                       "\nThe University of Arizona\nhttps://cals.arizona.edu/climate/\nData Source: NOAA MPE Analysis\nhttps://water.weather.gov/precip/"))+
        #   theme(plot.title=element_text(size=14, face = "bold"))
        # 
        ...
        # 
        # # write out file
        # jpeg(paste0("/home/crimmins/RProjects/StateMonsoonMaps/maps/",stateAbb[k],"/avg_",stateAbb[k],"_total_precip2.jpg"),
        #     width = 8, height = 8, units = "in", res = 300L)
        # #grid.newpage()
        # print(p, newpage = FALSE)
        # dev.off()
        ...
        # final_plot <- image_composite(plot, logo, offset = "+110+2150")
        # # And overwrite the plot without a logo
        # image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/maps/",stateAbb[k],"/avg_",stateAbb[k],"_total_precip2.jpg"))
        #####
        ...
        p<-p+geom_polygon(data = goc, 
            ...
            final_plot <- image_composite(plot, logo, offset = "+110+2150")
            # And overwrite the plot without a logo
            image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_TotalPrecipCat.png"))
        ...
        p<-p+geom_polygon(data = goc, 
            ...
            final_plot <- image_composite(plot, logo, offset = "+110+2150")
            # And overwrite the plot without a logo
            image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_PercPrecip.png"))
          ...
          p<-p+geom_polygon(data = goc, 
              ...
              final_plot <- image_composite(plot, logo, offset = "+110+2150")
              # And overwrite the plot without a logo
              image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_PercentDays.png"))
          ...
          p<-p+geom_polygon(data = goc, 
              ...
              final_plot <- image_composite(plot, logo, offset = "+110+2150")
              # And overwrite the plot without a logo
              image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_IntensityIndex.png"))
          ...
          p<-p+geom_polygon(data = goc, 
              ...
              final_plot <- image_composite(plot, logo, offset = "+110+2150")
              # And overwrite the plot without a logo
              image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_MaxPrecip.png"))
          ...
          p<-p+geom_polygon(data = goc, 
              ...
              final_plot <- image_composite(plot, logo, offset = "+110+2150")
              # And overwrite the plot without a logo
              image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_DaysSince.png"))
          ...
          p<-p+geom_polygon(data = goc, 
              ...
              final_plot <- image_composite(plot, logo, offset = "+110+2150")
              # And overwrite the plot without a logo
              image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_LatestDay.png"))
          ...
          #                color = 'black', size = 1)
           ...
           p<-p+geom_polygon(data = goc, 
              ...
              # And overwrite the plot without a logo
              image_write(final_plot, paste0("/home/crimmins/RProjects/StateMonsoonMaps/historic/",currYR,"/",stateAbb[k],"/",stateAbb[k],"_Monsoon_AllDaysGrid.png"))
          ...
          render(paste0('/home/crimmins/RProjects/StateMonsoonMaps/',stateAbb[k],'_createThumbsHTML_hist.Rmd'), output_file=paste0('dailyPrecip_',stateAbb[k],'.html'),
                 output_dir=paste0('/home/crimmins/RProjects/StateMonsoonMaps/historic/',currYR,"/",stateAbb[k],"/"), clean=TRUE)
          plots <- list.files(paste0('/home/crimmins/RProjects/StateMonsoonMaps/historic/',currYR,"/",stateAbb[k],"/daily_",stateAbb[k]))
          file.remove(paste0('/home/crimmins/RProjects/StateMonsoonMaps/historic/',currYR,"/",stateAbb[k],"/daily_",stateAbb[k],"/",plots))
         
          # copy markdown file to new historical directory
          file.copy(from = paste0('/home/crimmins/RProjects/StateMonsoonMaps/maps/',stateAbb[k],"/",stateAbb[k],'MonsoonTemplate.Rmd'), 
                    to =paste0('/home/crimmins/RProjects/StateMonsoonMaps/historic/',currYR,"/",stateAbb[k],"/",stateAbb[k],'MonsoonTemplate.Rmd'))
          file.copy(from = '/home/crimmins/RProjects/StateMonsoonMaps/maps/AZ/GitHub-Mark-32px.png',
                    to =paste0('/home/crimmins/RProjects/StateMonsoonMaps/historic/',currYR,"/",stateAbb[k],"/GitHub-Mark-32px.png"))
           
          # create Website with markdown ----
          render(paste0('/home/crimmins/RProjects/StateMonsoonMaps/historic/',currYR,"/",stateAbb[k],"/",stateAbb[k],'MonsoonTemplate.Rmd'), output_file=paste0(tolower(stateAbb[k]),'_monsoon.html'),
                 output_dir=paste0('/home/crimmins/RProjects/StateMonsoonMaps/historic/',currYR,"/",stateAbb[k],"/"), clean=TRUE)
          # ----  
